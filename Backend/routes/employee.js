...
router.get('/viewemployee',(req,res)=>{
    ...
    let  sql = `call v1_0_2_u_viewemployee()`;
...
router.post('/addFormEmployee',(req,res)=>{
    ...
    let sql = `call v1_0_2_u_addemployee(?,?,?,?,?,?,?)`;
    
    db.query(sql,[employeeNumber,firstName,lastName,email,officeCode,jobTitle,phoneNumber],
      (err,results)=>{
          if (err){
            console.log(err)
            res.send({state:"400",msg:"Error in database operation !!"});
